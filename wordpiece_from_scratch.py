...
for word in word_freqs.keys():
    ...
    for letter in word[1:]:  # 不是单词的第一个字母
        if (
            f"##{letter}" not in alphabet
        ):  # f"{letter}" 是格式化的语法，用 letter 变量的真实值来替代 {letter}
...
vocab = [
    ...
    "[SEP]",
    "[MASK]",
] + alphabet.copy()  # add special token
...
def compute_pair_scores(splits):
    ...
    扫描整个word_freqs中的word，然后每个word的split列表依次计算相邻的token pair的概率
    也就是除了统计pair的频率 pair_freqs 外，还要统计每个 token 的频率 letter_freqs
...
# 如BPE算法一样，高效的实现，可以将 merge_pair 和 compute_pair_scores 合并在一起计算
# 当前有 splits中的 "a b"合并时，就把 letter_freqs 中对应 a 和 b的频率减1，然后对应合并的 pair 加1
# 每次 "a b" 的合并应用时，就把它从 pair_freqs 删除，同时也要新增新的 （x,ab）和（ab,y）这样的pair
...
while len(vocab) < vocab_size:
    ...
    print(
        "\t Top3 Pair scores:%s" % sorted(scores.items(), key=lambda x: -x[1])[:3]
    )  # 得分降序排列
    current_pair = find_max_score(scores)
    new_subword = "%s%s" % (
        current_pair[0],
        current_pair[1][2:] if current_pair[1].startswith("##") else current_pair[1],
    )
